...
- There is **no daemon-side persistence privacy toggle yet**.
...
4. **Add a persistence privacy toggle in the daemon path** so settings can disable or narrow terminal
...
| **11** | **Not needed (removed)**       | Search excerpts / FTS and daemon cleanup/retention. The daemon-side implementation (indexing, retention, reaping) is complete. By design, there is no search UI, no session history panel, no clear-history buttons — persistence is invisible. Retention settings auto-clean old sessions; the user sees their terminals, not session metadata.                                                                                                                                                                                                                                                                              | N/A       |
...
**Total**: dominated by stress testing, adapter hardening, privacy controls, and VT snapshot
