...
test(
  'TaodClient restarts an owned real daemon after process exit',
  { skip: process.platform === 'win32' ? 'taod lifecycle integration is POSIX-only' : false },
  async (context) => {
    const binaryPath = findTaodBinary()
    if (!binaryPath) {
      context.skip('taod binary not found; run pnpm --filter @tao/desktop build first')
      return
    }

    const previousHome = process.env.HOME
    const previousTaodPath = process.env.TAOD_PATH
    const previousAdapterDir = process.env.TAOD_ADAPTER_DIR
    const home = mkdtempSync(resolve(tmpdir(), 'tao-real-lifecycle-'))
    process.env.HOME = home
    process.env.TAOD_PATH = binaryPath
    process.env.TAOD_ADAPTER_DIR = resolve(desktopRoot, 'out/adapters')

    const client = new TaodClient({
    ...
    try {
      ...
      if (previousTaodPath === undefined) delete process.env.TAOD_PATH
      else process.env.TAOD_PATH = previousTaodPath
      if (previousAdapterDir === undefined) delete process.env.TAOD_ADAPTER_DIR
      else process.env.TAOD_ADAPTER_DIR = previousAdapterDir
    }
  },
)

test(
  'TaodClient dispose preserves a detached daemon for app quit semantics',
  { skip: process.platform === 'win32' ? 'taod lifecycle integration is POSIX-only' : false },
  async (context) => {
    const binaryPath = findTaodBinary()
    if (!binaryPath) {
    ...
    const previousTaodPath = process.env.TAOD_PATH
    const previousAdapterDir = process.env.TAOD_ADAPTER_DIR
    const home = mkdtempSync(resolve(tmpdir(), 'tao-detached-quit-'))
    process.env.HOME = home
    process.env.TAOD_PATH = binaryPath
    process.env.TAOD_ADAPTER_DIR = resolve(desktopRoot, 'out/adapters')
...
test(
  'TaodClient records failure and restarts when owned daemon exits mid-request',
  {
    skip:
  ...
  async (context) => {
    ...
    const previousTaodPath = process.env.TAOD_PATH
    const previousAdapterDir = process.env.TAOD_ADAPTER_DIR
    const home = mkdtempSync(resolve(tmpdir(), 'tao-mid-request-'))
    process.env.HOME = home
    process.env.TAOD_PATH = binaryPath
    process.env.TAOD_ADAPTER_DIR = resolve(desktopRoot, 'out/adapters')
