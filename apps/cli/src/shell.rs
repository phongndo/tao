...
_tao() {{
  local -a commands shells wt_names
  commands=(
    ...
    'handoff:Move branch between worktree and local checkout'
    'ls:List worktrees'
...
complete -c tao -n 'not __fish_seen_subcommand_from tui review new cd local handoff ls path rm prune init completion help' -a handoff -d 'Move branch between worktree and local checkout'
